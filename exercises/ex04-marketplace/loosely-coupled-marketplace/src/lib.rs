...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn buy(
			...
			// get the amount of this specific NFT owned by the seller, through the Resource type and its Sellable trait
			let owned: u128 = T::Resource::amount_owned(nft_id, seller.clone());
			...
			// transfer the amount of currency owed from the buyer to the seller
			T::Currency::transfer(&buyer, &seller, total_to_pay, KeepAlive)?;

			// transfer amount of nft_id from the seller to the buyer
