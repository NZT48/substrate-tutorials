...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + scale_info::TypeInfo + pallet_marketplace_nfts::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn buy(
			...
			// call the pallet_marketplace_nft transfer function
			pallet_marketplace_nfts::Pallet::<T>::unchecked_transfer(nft_id, seller.clone(), buyer.clone(), amount);
